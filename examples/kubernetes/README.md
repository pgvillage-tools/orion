...
In this example you'll see how orion can provide an high available postgreSQL cluster inside kubernetes.
...
Prebuilt images are available on the dockerhub, the images' tags are the orion release version plus the postgresql version (for example v0.12.0-pg10).

**NOTE**: These images are **example** images provided for quickly testing orion. In production you should build your own image customized to fit your needs (adding postgres extensions, backup tools/scripts etc...).
...
For example, if you want to build an image named `orion:master-pg10` that uses postgresql 10 you should execute:

```
make PGVERSION=10 TAG=orion:master-pg10 docker
```

Once the image is built you should push it to the docker registry used by your kubernetes infrastructure.

The provided example uses `ghcr.io/pgvillage-tools/orion-keeper:17-latest`
...
* The cluster name is `kube-orion`. It's set in the various `orion-cluster` labels and in the component `--cluster-name` option. The labels and the `--cluster-name` option must be in sync.
...
If your k8s cluster has RBAC enabled you should create a role and a rolebinding to a service account. As an example take a look at the provided [role](role.yaml) and [role-binding](role-binding.yaml) example definitions that define a `orion` role bound to the `default` service account in the `default` namespace.

### Initialize the cluster

All the orion components wait for an existing clusterdata entry in the store. So the first time you have to initialize a new cluster. For more details see the [cluster initialization doc](/doc/initialization.md). You can do this step at every moment, now or after having started the orion components.

You can execute orionctl in different ways:

* as a one shot command executed inside a temporary pod:

```
kubectl run -i -t orionctl --image=ghcr.io/pgvillage-tools/orion-keeper:17-latest --restart=Never --rm -- /usr/local/bin/orionctl --cluster-name=kube-orion --store-backend=kubernetes --kube-resource-kind=configmap init
```

* from a machine that can access the store backend:

```
orionctl --cluster-name=kube-orion --store-backend=kubernetes --kube-resource-kind=configmap init
```

* later from one of the pods running the orion components.


### Create the sentinel(s)

```
kubectl create -f orion-sentinel.yaml
```

This will create a deployment that defines 2 replicas for the orion sentinel. You can change the number of replicas in the deployment definition (or scale it with `kubectl scale`).
...
### Create the orion keepers statefulset
...
This will define a statefulset that will create 2 orion-keepers.
...
```
kubectl create -f orion-proxy.yaml
```

This will create a deployment that defines 2 replicas for the orion proxy. You can change the number of replicas in the deployment definition (or scale it with `kubectl scale`).
...
```
kubectl create -f orion-proxy-service.yaml
```
...
The password for the orion user will be the value specified in your `secret.yaml` above (or `password1` if you did not change it).

```
psql --host orion-proxy-service  --port 5432 postgres -U orion -W
Password for user orion:
...
orion-keeper-0                    1/1       Running   0          5m
orion-keeper-1                    1/1       Running   0          5m
orion-proxy-fd7c9b4bd-89c9z       1/1       Running   0          5m
orion-proxy-fd7c9b4bd-pmj86       1/1       Running   0          5m
orion-sentinel-5c76865bd5-bc9n2   1/1       Running   0          5m
orion-sentinel-5c76865bd5-fmqts   1/1       Running   0          5m
```
...
```
kubectl delete statefulset orion-keeper --cascade=false
kubectl delete pod orion-keeper-0
```
...
You can add additional orion keepers increasing the replica count in the statefulset. Shrinking the statefulset should be done very carefully or you can end in a situation where the current master pod will be removed and the remaining keepers cannot be elected as master because not in sync.

### Scale your cluster sentinels and proxies

You can increase/decrease the number of orion sentinels and proxies:

```
kubectl scale --replicas=3 deployment orion-sentinel
```

```
kubectl scale --replicas=3 deployment orion-proxy
```
